...
const SetupForm: React.FC<SetupFormProps> = ({ onSubmit, isLoading }) => {
  ...
  return (
    <div className="z-10 relative w-full max-w-4xl p-3 sm:p-5 md:p-8 glass-panel rounded-xl sm:rounded-2xl shadow-2xl my-4 mx-2 sm:mx-0 max-h-[90vh] sm:max-h-[95vh] overflow-y-auto custom-scrollbar">
      ...
      <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-5 md:space-y-6">
        ...
        <div className="flex bg-slate-900/60 p-1 sm:p-1.5 rounded-full border border-white/10 mx-auto w-full sm:max-w-xs shadow-[inset_0_2px_4px_rgba(0,0,0,0.3)]">
            <button
                type="button"
                onClick={() => {
                    handleInteraction();
                ...
                className={`flex-1 flex items-center justify-center gap-1 sm:gap-2 py-2 sm:py-2.5 rounded-full text-[10px] sm:text-sm tracking-wide transition-all duration-300 ${
                    formData.relationship === 'partner' 
                    ? 'bg-gradient-to-r from-rose-600 to-pink-600 text-white shadow-lg shadow-rose-900/20' 
                    : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}
            >
                <Heart size={12} className="sm:w-[14px] sm:h-[14px] flex-shrink-0" fill={formData.relationship === 'partner' ? "currentColor" : "none"} /> 
                <span className="hidden sm:inline">Partner</span>
                <span className="sm:hidden text-[9px]">Me</span>
            </button>
            <button
                type="button"
                onClick={() => {
            ...
            >
                ...
                <span className="sm:hidden text-[9px]">BF</span>
            </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-2 sm:gap-3 md:gap-4">
             <div className="sm:col-span-1 md:col-span-5">
                <label className="block text-[8px] sm:text-[10px] uppercase tracking-widest text-slate-400 mb-1.5 sm:mb-2 ml-1">Your Name</label>
                <input
                    required
                    type="text"
                    onFocus={handleInteraction}
                    className="w-full bg-black/20 border border-white/10 rounded-lg p-2.5 sm:p-3 text-sm sm:text-base text-white focus:outline-none focus:border-rose-500/50 focus:bg-white/5 transition-colors placeholder-slate-600 font-sans"
            ...
            <div className="sm:col-span-1 md:col-span-5">
                <label className="block text-[8px] sm:text-[10px] uppercase tracking-widest text-slate-400 mb-1.5 sm:mb-2 ml-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-3 md:gap-4">
            <div>
              ...
              <textarea
                required
                onFocus={handleInteraction}
                className="w-full bg-black/20 border border-white/10 rounded-lg p-3 sm:p-4 text-white focus:outline-none focus:border-rose-500/50 focus:bg-white/5 transition-colors h-24 sm:h-28 md:h-32 resize-none placeholder-slate-600 text-sm leading-relaxed font-sans"
            ...
            <div className="flex flex-col">
                <div className="flex justify-between items-center mb-1.5 sm:mb-2 ml-1">
                    <label className="block text-[8px] sm:text-[10px] uppercase tracking-widest text-slate-400">Personal Note</label>
                    <span className={`text-[7px] sm:text-[9px] ${isNearLimit ? 'text-rose-400' : 'text-slate-500'}`}>
                ...
                <textarea
                    ...
                    className={`w-full flex-grow bg-white/5 border rounded-lg p-3 sm:p-4 text-slate-200 focus:outline-none focus:border-rose-500/50 transition-colors resize-none placeholder-white/10 font-serif italic text-sm ${isNearLimit ? 'border-rose-500/50' : 'border-white/10'}`}
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {(['romantic', 'funny', 'poetic', 'nerdy', 'roast'] as const).map((v) => (
              <button
                ...
                className={`py-1.5 sm:py-2 px-2 sm:px-3 md:px-4 rounded-lg text-[8px] sm:text-xs font-medium uppercase tracking-wider border transition-all whitespace-nowrap ${
                  formData.vibe === v 
        ...
        >
          {isLoading ? (
            <>
              <Loader2 className="animate-spin text-white/50 flex-shrink-0 sm:w-[18px] sm:h-[18px]" size={16} /> 
              <span className="hidden sm:inline">Connecting to Cosmos...</span>
              <span className="sm:hidden text-[10px]">Generating...</span>
            </>
          ) : (
            <>
                <Star size={14} className="sm:w-4 sm:h-4 text-yellow-200 group-hover:rotate-180 transition-transform duration-500 flex-shrink-0" /> 
                <span className="hidden sm:inline">Generate Experience</span>
                <span className="sm:hidden text-[10px]">Generate</span>
            </>
          )}
        </button>
      </form>
